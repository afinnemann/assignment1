...
corrgram(da2[,7:14], col.regions = colorRampPalette(c("dodgerblue4",'dodgerblue','white', 'gold',"firebrick4")),cor.method='pearson')


cortest.bartlett(da2[,7:14]) # yields significant result, Chisq(28) = 2626, p = 0. In other words the correlation matrix is significantly different from an identity-matrix


#Principle component analysis:
pca <- principal(da2[,7:14], nfactors = length(7:14), scores = TRUE, rotate = "oblimin")

#Scree plot
plot(pca$values, type = "b")
#keeping 4 factors, although the first component seems to reflect most of covariance.
pc1 <- principal(da2[,7:14], nfactors = 4, scores = TRUE, rotate = "oblimin")
...
pca_model1 <- lmer(CHI_MLU ~  TC1 +(1+SUBJ|VISIT), pca_data)
pca_model2 <- lmer(CHI_MLU ~  TC1 + TC2 +(1+SUBJ|VISIT), pca_data)
pca_model3 <- lmer(CHI_MLU ~  TC1 + TC2 + TC3 +(1+SUBJ|VISIT), pca_data)

base_pca <- lmer(CHI_MLU ~ MOT_MLU*types_CHI*VISIT + VerbalIQ2 +(1+SUBJ|VISIT), pca_data)

anova(base_pca, pca_model1, pca_model2, pca_model3)
...
the_beauty <- pca_model2 <- lmer(CHI_MLU ~  RC1 + RC2 +(1+SUBJ|VISIT), pca_data)
