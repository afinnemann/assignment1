...
```{r}
demo_da <- read.delim("demo_train(1).csv", sep = ",", header = T)
lu_da <- read.delim ("LU_train.csv", sep =",", header = T)
token_da <- read.delim ("token_train.csv", sep =",", header = T)
```
...
```{r}
demo_sub <- demo_da %>% 
  select(SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS,  MullenRaw, ExpressiveLangRaw) %>% 
  rename(nonVerbalIQ = MullenRaw)
demo_sub <- demo_sub %>% 
  rename(VerbalIQ = ExpressiveLangRaw)

lu_sub <- lu_da %>% 
  select(SUBJ, VISIT, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
token_sub <- token_da %>% 
...
```{r}
all <- merge(demo_sub, lu_sub, by = c("SUBJ","VISIT"), all.x = T)
all <- merge(all, token_sub, by = c("SUBJ","VISIT"), all.x = T)
...
```{r}
#write.csv(all2, "autism_data.csv")
```
...
chi_da <- all2 %>% 
  ...
  select(SUBJ,CHI_MLU, tokens_CHI, VISIT)
...
```{r}
#I assume that this assigment should be solved for the children
chi_da <- chi_da %>% 
  group_by(VISIT) %>% 
  mutate(avg_words_visit = sum(tokens_CHI, na.rm = T)/ length(tokens_CHI))

#I don't see why it makes sense to use summarise, if want to add a new column.
chi_da  <- chi_da %>% 
  group_by(SUBJ) %>% 
  mutate(words_per_chi = sum(tokens_CHI, na.rm = T) / length(tokens_CHI))

#summarise would make sense for a new df
results_df <- chi_da %>% 
  group_by(SUBJ) %>% 
  summarise(visit_mean_words = sum(tokens_CHI, na.rm = T) / length(VISIT))
#is it possible to exclude 

#answer 3: nr. of words doesn't measure the quality of childrens vocabulary, only that they are talkative if we assume that they have been doing the same activities (if the children have been during different activities, the result can be explained by that)
